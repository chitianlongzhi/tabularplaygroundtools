...
class tabularplaygroundtools:
  def __init__(self, param):
    ...
    if self.param['type'] == 'LSTM':
      ...
      print(tab+'    verbose=1')
      print(tab+')')
      print(tab+'pred += model.predict(X_test)[0]'.format(self.param['target']))
    if self.param['type'] == 'Tensorflow':
      print(tab+'history = model.fit(')
      ...
      print(tab+')')
      print(tab+'pred += model.predict(X_test)'.format(self.param['target']))
    if self.param['type'] == 'PyTorch':
      ...
      print(tab+'pred += model(X_test_tensor).detach().numpy()'.format(self.param['target']))
    if self.param['type'] == 'XGBoost':
      print(tab+'model.fit(')
      print(tab+'    X_train, y_train,')
    ...
    if 'KFold' in self.param and self.param['KFold']:
      ...
      print('test[\'{}\'] = pred'.format(self.param['target']))
    print('############################################################')
