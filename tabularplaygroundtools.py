...
class tabularplaygroundtools:
  def __init__(self, param):
    ...
    if param['type'] == 'PyTorch':
      print('# reshape for PyTorch')
      print('import torch')
      print('device=\'cuda\' if torch.cuda.is_available() else \'cpu\'')
      print('X_train_tensor = torch.Tensor(np.float32(X_train)).to(device)')
      ...
      print('valid_loader = torch.utils.data.DataLoader(valid_dataset, batch_size=512, shuffle=True)')
      print('# PyTorch model')
      ...
      print('%%time')
      print('# fitting')
      print('for epoch in range(1000):')
      print('    model.train()')
      print('    loss_train_list=[]')
      print('    for X_train_tensor, y_train_tensor in train_loader:')
      print('        optimizer.zero_grad()')
      print('        o_train_tensor = model(X_train_tensor)')
      print('        loss_train = criterion(o_train_tensor, y_train_tensor)')
      print('        loss_train.backward()')
      print('        optimizer.step()')
      print('        loss_train_list.append(loss_train.item())')
      print('    model.eval()')
      print('    loss_valid_list=[]')
      print('    for X_valid_tensor, y_valid_tensor in train_loader:')
      print('        o_valid_tensor = model(X_valid_tensor)')
      print('        loss_valid = criterion(o_valid_tensor, y_valid_tensor)')
      print('        loss_valid_list.append(loss_valid.item())')
      print('    if epoch%100==0:')
      print('        print(\'{} train loss {}, valid loss {}\'.format(epoch, np.mean(loss_train_list), np.mean(loss_valid_list)))')
      print('# predicting')
      print('pred = model(X_test_tensor).detach().numpy()')
      print('test[\'{}\'] = model(X_test_tensor).detach().numpy()'.format(param['target']))
